...
try:
    ...
    logger.error(
        "Could not import model. Make sure you are running from the project root."
    )
    sys.exit(1)
...
def send_alert(customer_id, risk_score, channels):
    ...
    if "email" in channels:
        logger.info(
            f"[EMAIL TRIGGERED]: Sent retention workflow email to account manager for {customer_id}"
        )
...
def run_batch_inference_and_alert(threshold=0.75, limit=10, channels=["slack"]):
    ...
    try:
        logger.info(
            f"Connecting to database to extract up to {limit} unscored users..."
        )
    ...
    logger.info(
        f"Batch job complete. Evaluated {len(df)} users. Sent {alerts_sent} alerts."
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=float,
        default=0.70,
        help="Churn probability threshold to trigger alert",
    )
    parser.add_argument(
        "--limit",
        type=int,
        default=50,
        help="Number of customers to score in this batch",
    )
    parser.add_argument(
        "--channels",
        nargs="+",
        default=["slack", "email"],
        help="Alert channels (slack, email)",
    )

    args = parser.parse_args()

    run_batch_inference_and_alert(
        threshold=args.threshold, limit=args.limit, channels=args.channels
    )
