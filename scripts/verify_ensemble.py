...
def verify_ensemble():
    ...
    if not os.path.exists(data_path):
        print(
            f"✗ Error: Data file {data_path} not found. Please ensure the dataset is in the data/ directory."
        )
        return
    ...
    with torch.no_grad():
        y_probs = (
            ensemble.predict_proba(X_test_tensor, X_test_seq).cpu().numpy().flatten()
        )
    ...
    if missing:
        ...
        print(
            "Recommendation: Run corresponding training scripts to generate these components."
        )
