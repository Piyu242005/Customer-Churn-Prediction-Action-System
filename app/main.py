...
st.set_page_config(
    page_title="Customer Churn Prediction & Action System", layout="wide"
)

st.title(" Customer Churn Prediction & Action System")
st.markdown(
    "Upload a customer churn dataset to predict churn risk using **XGBoost**. The system features real-time inference, actionable insights mapped to retention strategies, and **SHAP** explainability to uncover why a customer might churn."
)
...
if uploaded_file is not None:
    ...
    if missing_feats:
        ...
        st.error(
            "Model artifacts not found. Please run the model training script `src/train_dashboard.py` first."
        )
    else:
        if st.button("Predict Real Churn Risk", type="primary"):
            with st.spinner("Running XGBoost inference and SHAP explanations..."):
                ...
                X_inference["TotalCharges"] = pd.to_numeric(
                    X_inference["TotalCharges"], errors="coerce"
                ).fillna(0)
                ...
                cat_cols = [
                    ...
                    if c in X_inference.columns
                ]
                X_inference = pd.get_dummies(
                    X_inference, columns=cat_cols, drop_first=True
                )
                ...
                top_drivers = [
                    expected_features[idx] if expected_features else str(idx)
                    for idx in top_driver_indices
                ]
                ...
                risk_filter = st.selectbox(
                    "Filter Table by Risk Level:", ["All", "High", "Medium", "Low"]
                )
                if risk_filter != "All":
                ...
                def highlight_risk(val):
                    color = (
                        "#ff4b4b"
                        if val == "High"
                        else "#ffa32f" if val == "Medium" else "#00cc96"
                    )
                    return f"color: {color}; font-weight: bold"

                disp_cols = [
                    c for c in ["customerID", "gender", "Partner"] if c in df.columns
                ]
                if not disp_cols:
                    disp_cols = list(df.columns[:2])
                disp_cols += [
                    "Churn_Prob_%",
                    "Risk_Level",
                    "Top_Reason",
                ...
                st.dataframe(
                    display_df[disp_cols].style.map(
