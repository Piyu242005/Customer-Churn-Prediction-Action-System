...
class FeatureEngineer:
    ...
    def plot_feature_importance(self, top_n=None, save_path=None):
        ...
        if self.feature_importance is None:
            print(
                "Error: Feature importance not calculated. Run calculate_feature_importance first."
            )
            return
    ...
    def recursive_feature_elimination(self, X, y, n_features_to_select=10, step=1):
        ...
        if self.feature_names:
            ...
            for i, (name, rank) in enumerate(
                zip(selected_names, rfe.ranking_[selected_indices]), 1
            ):
    ...
    def apply_smote(
        self, X, y, sampling_strategy="auto", method="smote", random_state=42
    ):
        """
        ...
        print(
            f"Resampled class distribution: {dict(zip(*np.unique(y_resampled, return_counts=True)))}"
        )
    ...
    def select_k_best_features(self, X, y, k=10, score_func="f_classif"):
        ...
        if self.feature_names:
            ...
            feature_scores = pd.DataFrame(
                {"feature": selected_names, "score": scores[selected_indices]}
    ...
    def plot_class_distribution(self, y, y_resampled=None, save_path=None):
        ...
        if y_resampled is not None:
            ...
            unique, counts = np.unique(y, return_counts=True)
            axes[0].bar(
                ["Active (0)", "Churned (1)"], counts, color=["#3498db", "#e74c3c"]
            )
            axes[0].set_title(
                "Original Class Distribution", fontsize=14, fontweight="bold"
            )
            ...
            axes[1].bar(
                ["Active (0)", "Churned (1)"], counts_r, color=["#3498db", "#e74c3c"]
            )
...
class FeatureSelector:
    ...
    def compare_selection_methods(self, X, y, k=10):
        ...
        results["random_forest"] = {
            ...
            "features": (
                [self.feature_names[i] for i in rf_indices]
                if self.feature_names
                else rf_indices
            ),
        }
        ...
        results["mutual_info"] = {
            ...
            "features": (
                [self.feature_names[i] for i in mi_indices]
                if self.feature_names
                else mi_indices
            ),
        }
        ...
        results["f_classif"] = {
            ...
            "features": (
                [self.feature_names[i] for i in f_indices]
                if self.feature_names
                else f_indices
            ),
        }
        ...
        results["rfe"] = {
            ...
            "features": (
                [self.feature_names[i] for i in rfe_indices]
                if self.feature_names
                else rfe_indices
            ),
        }
        ...
        consensus_features = [
            feat for feat, count in feature_counts.items() if count >= 3
        ]

        print(f"\n{'='*60}")
        print(
            f"Consensus Features (appearing in ≥3 methods): {len(consensus_features)}"
        )
        print(f"{'='*60}")
    ...
    def plot_feature_selection_comparison(self, save_path=None):
        ...
        colors = [
            "#27ae60" if x >= 3 else "#f39c12" if x == 2 else "#95a5a6"
...
def apply_advanced_feature_engineering(
    ...
    if use_feature_selection and n_features:
        ...
        importance_df.head(n_features).index.tolist()

        # Get actual column indices
        if feature_names:
            selected_indices = [
                feature_names.index(importance_df.iloc[i]["feature"])
...
if __name__ == "__main__":
    ...
    # Calculate feature importance
    importance_df = engineer.calculate_feature_importance(
        X_train_np, y_train_np, method="random_forest"
    )
    engineer.plot_feature_importance(top_n=10, save_path="feature_importance.png")

    # Compare selection methods
    selector = FeatureSelector(feature_names)
    results, consensus = selector.compare_selection_methods(
        X_train_np, y_train_np, k=10
    )
    selector.plot_feature_selection_comparison(
        save_path="feature_selection_comparison.png"
    )

    # Apply SMOTE
    X_train_balanced, y_train_balanced = engineer.apply_smote(X_train_np, y_train_np)
    engineer.plot_class_distribution(
        y_train_np, y_train_balanced, save_path="class_distribution.png"
    )
