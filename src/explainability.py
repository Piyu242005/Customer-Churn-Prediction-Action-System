...
class ChurnExplainer:
    ...
    def plot_shap_force(
        self, shap_values, X_test, explainer, sample_idx=0, save_path=None
    ):
        """
    ...
    def get_top_features_for_prediction(
        self, shap_values, X_test, sample_idx=0, top_n=5
    ):
        """
    ...
    def explain_prediction(self, shap_values, X_test, y_test, sample_idx=0):
        ...
        print("=" * 60)
        print(
            f"True Class:        {'Churned' if true_class == 1 else 'Active'} ({true_class})"
        )
        print(
            f"Predicted Class:   {'Churned' if prediction_class == 1 else 'Active'} ({prediction_class})"
        )
        print(
            f"Churn Probability: {prediction_proba:.4f} ({prediction_proba*100:.2f}%)"
        )
        print(
            f"Prediction:        {'✓ CORRECT' if prediction_class == true_class else '✗ INCORRECT'}"
        )

        # Get top contributing features
        top_features = self.get_top_features_for_prediction(
            shap_values, X_test, sample_idx, top_n=10
        )
        ...
        for i, row in enumerate(top_features.itertuples(), 1):
            ...
            print(
                f"{i:<6} {row.feature:<30} {row.feature_value:<12.4f} {row.shap_value:>+8.4f} {impact}"
            )
...
class LIMEExplainer:
    ...
    def __init__(
        self, model, feature_names, class_names=["Active", "Churned"], device="cpu"
    ):
        """
    ...
    def explain_instance(self, X_test, sample_idx=0, num_features=10):
        ...
        explanation = self.explainer.explain_instance(
            X_test[sample_idx], self._predict_fn, num_features=num_features
        )
    ...
    def plot_lime_explanation(self, explanation, save_path=None):
        ...
        plt.title(
            "LIME Feature Importance for Prediction", fontsize=14, fontweight="bold"
        )
        plt.tight_layout()
...
def generate_shap_report(
    ...
    shap_values, shap_explainer = explainer.compute_shap_values(
        X_train_sample, X_test_sample, max_samples=100
    )
    ...
    tp_indices = np.where(
        (y_test_np == 1)
        & (
            explainer._predict_fn(
                X_test_sample
                if isinstance(X_test_sample, np.ndarray)
                else X_test_sample.numpy()
            ).flatten()
            >= 0.5
        )
    )[0]
    if len(tp_indices) > 0:
        ...
        explainer.explain_prediction(
            shap_values, X_test_sample, y_test_np, sample_idx=tp_indices[0]
        )
    ...
    if len(fp_indices) > 0:
        ...
        explainer.explain_prediction(
            shap_values, X_test_sample, y_test_np, sample_idx=fp_indices[0]
        )
        explainer.plot_shap_waterfall(
            shap_values,
            X_test_sample,
...
def generate_lime_report(
    ...
    sample_indices = np.random.choice(
        len(X_test_np), min(num_samples, len(X_test_np)), replace=False
    )
    ...
    for i, idx in enumerate(sample_indices):
        ...
        explanation = lime_explainer.explain_instance(
            X_test_np, sample_idx=idx, num_features=10
        )
        lime_explainer.plot_lime_explanation(
            explanation, save_path=f"{save_dir}lime_explanation_sample_{idx}.png"
        )
...
def generate_business_insights(shap_values, X_test, feature_names, save_path=None):
    ...
    for i, row in enumerate(importance_df.head(10).itertuples(), 1):
        ...
        print(
            f"   Impact Score: {row.Average_Impact:.4f} ({row.Impact_Percentage:.2f}% of total)"
        )
    ...
    for feature in importance_df.head(5)["Feature"]:
        ...
        print(f"\n{feature}:")
        print(
            f"   Increases churn risk: {positive_impact} samples ({positive_impact/len(feature_shap)*100:.1f}%)"
        )
        print(
            f"   Decreases churn risk: {negative_impact} samples ({negative_impact/len(feature_shap)*100:.1f}%)"
        )
    ...
    if save_path:
        with open(save_path, "w") as f:
            ...
            for i, row in enumerate(importance_df.head(10).itertuples(), 1):
                f.write(
                    f"{i}. {row.Feature}: {row.Average_Impact:.4f} ({row.Impact_Percentage:.2f}%)\n"
                )
            ...
            f.write("-" * 60 + "\n")
            f.write(
                "1. Focus retention efforts on customers with high-risk feature values\n"
            )
...
def main():
    ...
    generate_business_insights(
        shap_values, X_test, feature_names, save_path="business_insights_report.txt"
    )
