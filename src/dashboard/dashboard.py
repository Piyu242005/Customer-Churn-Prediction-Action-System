...
def load_resources():
    ...
    try:
        ...
        for path_str in [
            ...
            "outputs/run_20260315_185202/models/mlp_churn_classifier.pth",
...
st.sidebar.markdown(
    "<br><hr style='border: 1px solid rgba(0, 212, 255, 0.3); margin-top:20px; margin-bottom:20px;'>",
    unsafe_allow_html=True,
)
st.sidebar.markdown(
    "<div style='font-size: 0.85rem; color:#9ca3af; margin-bottom:10px;'><b>SYSTEM HEALTH</b></div>",
    unsafe_allow_html=True,
)
...
st.sidebar.markdown(
    f"{health_dot(model is not None)} <span style='color:white; font-size:0.9rem;'>{'Model Loaded' if model else 'Model Not Found'}</span>",
    unsafe_allow_html=True,
)
st.sidebar.markdown(
    f"{health_dot(scaler is not None)} <span style='color:white; font-size:0.9rem;'>{'Scaler Ready' if scaler else 'Scaler Not Found'}</span>",
    unsafe_allow_html=True,
)
st.sidebar.markdown(
    f"{health_dot(encoders is not None)} <span style='color:white; font-size:0.9rem;'>{'Encoders Ready' if encoders else 'Encoders Not Found'}</span>",
    unsafe_allow_html=True,
)
...
def show_about_dialog():
    ...
    techs = ["PyTorch", "Flask", "Streamlit", "Scikit-learn", "SHAP", "Docker"]
    tech_pills = " ".join([f"""<span style="
            background: rgba(0,212,255,0.08);
            border: 1px solid rgba(0,212,255,0.2);
            border-radius: 20px;
            padding: 4px 12px;
    ...
    stat_rows = "".join([f"""<div style="
        ...
        ">• {s}</div>""" for s in stats])

    about_html = about_html.format(tech_pills=tech_pills, stat_rows=stat_rows)
...
if model is None:
    st.error(
        "❌ **Primary Model (.pth) not found.** Please run `python src/train.py` or the training pipeline to generate model artifacts first."
    )
...
if selected_page == "🏢 Executive Overview":
    with st.spinner("🧠 Piyush Ramteke's Churn AI · Crunching the numbers..."):
        ...
        with c1:
            st.markdown(
                render_metric_card(
                    "Accuracy", "89%", "↑ +11% vs baseline", border_color="#00d4ff"
                ),
                unsafe_allow_html=True,
            )
        with c2:
        ...
        with c1:
            ...
            cm_text = [
                ["TN = 229<br>(19%)", "FP = 28<br>(2%)"],
                ["FN = 113<br>(9%)", "TP = 829<br>(70%)"],
            ]
            ...
            fig1 = plgo.Figure(
                data=plgo.Heatmap(
                    ...
                    colorscale=[[0, "#1e2130"], [0.5, "#0891b2"], [1, "#00ff88"]],
                    showscale=False,
                )
            )
            fig1.update_layout(
                template="plotly_dark",
                title="Test Set Performance (n=1,199)",
            ...
            st.markdown(
                "<div style='text-align:center; color:#00ff88; font-weight:bold;'>✓ 829/942 churned customers correctly identified (88% recall)</div>",
                unsafe_allow_html=True,
            )
        ...
        with c2:
            ...
            fig3 = px.pie(
                ...
                values=[257, 942],
                hole=0.5,
                color_discrete_sequence=["#00ff88", "#ff4b6e"],
            )
            fig3.update_layout(
                template="plotly_dark",
            ...
            st.markdown(
                "<div style='text-align:center; color:#9ca3af;'>Distribution of the 1,199 test targets evaluated</div>",
                unsafe_allow_html=True,
            )
...
# =============================================================================
    ...
    with st.spinner("🧠 Piyush Ramteke's Churn AI · Consulting the oracle..."):
        ...
        st.markdown(
            "<div style='color:#9ca3af; margin-bottom: 20px;'>Input customer metrics to predict absolute churn probability dynamically using the trained MLP weights.</div>",
            unsafe_allow_html=True,
        )
        ...
        with st.form("inference_form"):
            ...
            with c1:
                ...
                total_revenue = st.number_input(
                    "Total Revenue ($)", 0.0, 50000.0, 1500.0
                )
                avg_revenue = st.number_input("Average Revenue ($)", 0.0, 5000.0, 300.0)
                std_revenue = st.number_input(
                    "Standard Deviation Revenue", 0.0, 1000.0, 50.0
                )
            ...
            with c2:
                ...
                category = st.selectbox(
                    "Product Category", cat_options["Product_Category"]
                )
                segment = st.selectbox(
                    "Customer Segment", cat_options["Customer_Segment"]
                )
        ...
        if avg_profit > avg_revenue:
            validation_warning = True
        if validation_warning:
            st.warning(
                "⚠️ Input mismatch: Please assure Profit components do not exceed Revenue bounds."
            )
        ...
        if submitted:
            ...
            with st.spinner(
                "🧠 Piyush Ramteke's Churn AI · Processing deep network inference..."
            ):
                ...
                with p_c2:
                    ...
                    if prob < 0.4:
                        st.markdown(
                            "<div class='risk-badge risk-low' style='font-size:1.5rem; width:fit-content;'>✓ LOW RISK PROFILE</div>",
                            unsafe_allow_html=True,
                        )
                        st.success(
                            "🟢 **Recommendation:** Continue normal marketing sequences. Customer is exhibiting stable behavior indicators."
                        )
                    elif prob < 0.7:
                        st.markdown(
                            "<div class='risk-badge risk-medium' style='font-size:1.5rem; width:fit-content;'>⚠️ MEDIUM RISK PROFILE</div>",
                            unsafe_allow_html=True,
                        )
                        st.warning(
                            "🟡 **Recommendation:** Send an automated targeted discount or check-in verification email. Monitor activity."
                        )
                    else:
                        st.markdown(
                            "<div class='risk-badge risk-high' style='font-size:1.5rem; width:fit-content;'>🚨 HIGH RISK PROFILE</div>",
                            unsafe_allow_html=True,
                        )
                        st.error(
                            "🔴 **Recommendation:** Immediate executive intervention required! Trigger aggressive retention campaign protocols immediately."
                        )

                # Shap Explanation Waterfall
                if model is not None:
                    st.markdown(
                        "### 🔍 Explainability: Top Feature Contributions (SHAP Waterfall)"
                    )
                    try:
                        ...
                        fig_shap.update_layout(
                            ...
                            xaxis_title="Impact on Prediction Value",
                            yaxis_title="",
                            margin=dict(l=200, r=50, t=10, b=40),
                            height=350,
                            xaxis=dict(
                        ...
                        st.error(
                            f"Could not generate localized SHAP explainability instance: {str(e)[:100]}"
                        )
...
# =============================================================================
    ...
    with st.spinner("🧠 Piyush Ramteke's Churn AI · Preparing bulk pipeline..."):
        ...
        st.markdown(
            "<div style='color:#9ca3af; margin-bottom: 20px;'>Process and analyze multiple customer records at scale through the predictive engine. Output standard CSV or robust PDF reporting.</div>",
            unsafe_allow_html=True,
        )
        ...
        with c1:
            ...
            st.download_button(
                ...
                df_template.to_csv(index=False).encode(),
        ...
        if uploaded_file is not None:
            ...
            if st.button("🚀 Run Batch Predictions", use_container_width=True):
                run_batch = True
                missing = [
                    col for col in feature_names_list if col not in df_batch.columns
                ]
                if missing:
                    st.error(
                        f"Missing required data columns mapping sequence: {', '.join(missing)}"
                    )
                    run_batch = False
                ...
                if run_batch:
                    ...
                    for i, row in df_batch.iterrows():
                        ...
                        progress.progress(
                            min((i + 1) / len(df_batch), 1.0),
                            text=f"Processing {i+1} of {len(df_batch)} records...",
                        )
                    ...
                    st.markdown(
                        "<div class='cyan-divider'></div>", unsafe_allow_html=True
                    )
                    ...
                    if high_risk > 0:
                        st.markdown(
                            f"<div style='padding:15px; border-radius:8px; border:1px solid #ff4b6e; background:rgba(255,75,110,0.1); margin-bottom:20px; color:#ff4b6e;'>🚨 <b>Alert:</b> Detected <b>{high_risk}</b> High Risk customers out of {len(results_df)} evaluations. Ensure immediate escalations are planned.</div>",
                            unsafe_allow_html=True,
                        )
                    else:
                        st.markdown(
                            f"<div style='padding:15px; border-radius:8px; border:1px solid #00ff88; background:rgba(0,255,136,0.1); margin-bottom:20px; color:#00ff88;'>✓ <b>Clean:</b> Detected <b>0</b> High Risk customers out of {len(results_df)} evaluations. Base is healthy.</div>",
                            unsafe_allow_html=True,
                        )

                    def color_rows(row):
                        if row["Prediction"] == "Churned":
                            return ["background-color: rgba(255, 75, 110, 0.2)"] * len(
                                row
                            )
                    ...
                    hist_fig = px.histogram(
                        ...
                        color="Prediction",
                        color_discrete_map={"Churned": "#ff4b6e", "Active": "#00ff88"},
                        nbins=20,
                    )
                    ...
                    with d_c2:
                        try:
                            ...
                            st.info(
                                "💡 To generate PDF summaries, make sure to add logic with reportlab or fpdf."
                            )
                        except ImportError:
                            st.info(
                                "💡 To generate PDF summaries, install: `pip install reportlab fpdf2`"
                            )
...
# =============================================================================
    ...
    with st.spinner("🧠 Piyush Ramteke's Churn AI · Loading parameter engine..."):
        ...
        st.markdown(
            "<div style='color:#9ca3af; margin-bottom: 20px;'>Dynamically visualize retention thresholds using live evaluation simulations based entirely on predefined customer archetypes.</div>",
            unsafe_allow_html=True,
        )
        ...
        with layoutA:
            ...
            with c1:
                ...
                total_revenue = st.number_input(
                    "Total Revenue", 0.0, 50000.0, sd[1], key="w2"
                )
                avg_revenue = st.number_input(
                    "Avg Revenue", 0.0, 5000.0, sd[2], key="w3"
                )
                std_revenue = st.number_input(
                    "Std Revenue", 0.0, 1000.0, sd[3], key="w4"
                )
                total_profit = st.number_input(
                    "Total Profit", 0.0, 20000.0, sd[4], key="w5"
                )
                days_since = st.slider(
                    "Days Since Purchase", 0, 365, int(sd[9]), key="w6"
                )
            with c2:
                avg_profit = st.number_input("Avg Profit", 0.0, 2000.0, sd[5], key="w7")
                avg_discount = st.slider(
                    "Avg Discount", 0.0, 1.0, float(sd[6]), key="w8"
                )
                ...
                purchase_freq = st.slider(
                    "Purchase Freq", 0.0, 1.0, float(sd[11]), key="w12"
                )
            with c3:
                region = st.selectbox(
                    "Region",
                    cat_options["Region"],
                    index=cat_options["Region"].index(sd[12]),
                    key="w13",
                )
                category = st.selectbox(
                    "Category",
                    cat_options["Product_Category"],
                    index=cat_options["Product_Category"].index(sd[13]),
                    key="w14",
                )
                segment = st.selectbox(
                    "Segment",
                    cat_options["Customer_Segment"],
                    index=cat_options["Customer_Segment"].index(sd[14]),
                    key="w15",
                )
                payment = st.selectbox(
                    "Payment",
                    cat_options["Payment_Method"],
                    index=cat_options["Payment_Method"].index(sd[15]),
                    key="w16",
                )
        ...
        with layoutB:
            ...
            if prob > 0.5:
                ...
                st.info(
                    "💡 **What changes would make this customer Active?**\n"
                ...
                st.markdown("##### Actionable Insights:")
                st.success(
                    "✓ Customer is currently behaving within the expected Active framework. Maintain standard retention operations."
                )
...
# =============================================================================
    ...
    with st.spinner("🧠 Piyush Ramteke's Churn AI · Fetching global telemetrics..."):
        ...
        st.markdown(
            "<div style='color:#9ca3af; margin-bottom: 20px;'>Live statistics across the current inference session and metadata corresponding to internal learning properties natively extracted.</div>",
            unsafe_allow_html=True,
        )
        ...
        with c1:
            st.markdown(
                render_metric_card(
                    "Total Session Inference",
                    str(st.session_state.predictions_count),
                    "Internal Runtime",
                    border_color="#00d4ff",
                    badge_bg="rgba(0, 212, 255, 0.2)",
                    badge_color="#00d4ff",
                ),
                unsafe_allow_html=True,
            )
        ...
        if st.session_state.prediction_history:
            avg_p = sum(
                x["probability"] for x in st.session_state.prediction_history
            ) / len(st.session_state.prediction_history)
            avg_r = sum(
                x["revenue"] for x in st.session_state.prediction_history
            ) / len(st.session_state.prediction_history)
        else:
            avg_p, avg_r = 0, 0
        ...
        with c2:
            st.markdown(
                render_metric_card(
                    "Avg Session Confidence",
                    f"{(avg_p*100):.1f}%",
                    "Aggregated Output",
                    border_color="#ffd93d",
                    badge_bg="rgba(255, 217, 61, 0.2)",
                    badge_color="#ffd93d",
                ),
                unsafe_allow_html=True,
            )
        ...
        with h1:
            ...
            if st.session_state.prediction_history:
                ...
                fig_hist = px.histogram(
                    ...
                    x="probability",
                    nbins=15,
                    title="Aggregate Output Distributions",
                )
                ...
                st.info(
                    "No predictions executed within this session. Execute requests on 'Single Inference' module."
                )
        ...
        with h2:
            ...
            if st.session_state.prediction_history:
                st.dataframe(
                    pd.DataFrame(st.session_state.prediction_history).tail(10),
                    use_container_width=True,
                    height=280,
                )
            else:
        ...
        with t1:
            ...
            fig_curve.add_trace(
                plgo.Scatter(x=x, y=loss, name="Train Loss", line=dict(color="#00ff88"))
            )
            fig_curve.add_trace(
                plgo.Scatter(
                    x=x, y=val_loss, name="Eval Loss", line=dict(color="#00d4ff")
                )
            )
            fig_curve.update_layout(
                ...
                plot_bgcolor="rgba(0,0,0,0)",
                height=300,
                yaxis_title="BCEWithLogitsLoss Output",
...
# =============================================================================
    ...
    with st.spinner("🧠 Piyush Ramteke's Churn AI · Calculating ROIs..."):
        ...
        st.markdown(
            "<div style='color:#9ca3af; margin-bottom: 20px;'>Project long-term retention recovery using deterministic threshold evaluation and variable acquisition frameworks constraints.</div>",
            unsafe_allow_html=True,
        )
        ...
        with c2:
            ...
            st.markdown(
                f"<div style='text-align:center; padding: 20px; background: rgba(0, 255, 136, 0.05); border: 1px solid rgba(0, 255, 136, 0.2); border-radius:12px; margin-top: 10px;'><h2 style='color:#00ff88; margin:0;'>📊 Total Estimated Annual Model Returns: ${(net_roi * 12):,.0f}</h2></div>",
                unsafe_allow_html=True,
            )
            ...
            with gx:
                fig2 = px.pie(
                    names=["Campaign Investment Cost", "Net Recovery ROI Margin"],
                    values=[campaign_cost, net_roi if net_roi > 0 else 0],
                    color_discrete_sequence=["#c084fc", "#00d4ff"],
                    hole=0.6,
                )
                fig2.update_layout(
                    title="Return Capital Breakdown",
                    template="plotly_dark",
                    paper_bgcolor="rgba(0,0,0,0)",
                    plot_bgcolor="rgba(0,0,0,0)",
                    height=350,
        ...
        intr_df = pd.DataFrame(
            [
                {
                    "Pipeline Parameter": "days_since_last_purchase",
                    "Calculated Core Meaning": "Recency of execution engagement metrics",
                },
                {
                    "Pipeline Parameter": "total_profit",
                    "Calculated Core Meaning": "Overall customer threshold profitability limits",
                },
                {
                    "Pipeline Parameter": "purchase_frequency",
                    "Calculated Core Meaning": "Continuous predictive alignment engagement consistency",
                },
                {
                    "Pipeline Parameter": "customer_lifetime_days",
                    "Calculated Core Meaning": "Extrapolated historical structural Loyalty indexes",
                },
                {
                    "Pipeline Parameter": "average_discount",
                    "Calculated Core Meaning": "Financial conversion elasticity metrics",
                },
            ]
        )
