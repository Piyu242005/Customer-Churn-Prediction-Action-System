...
def plot_roc_curve(y_true, y_pred_proba, save_path=None):
    ...
    plt.plot(
        ...
        tpr,
        linewidth=2,
        label=f"MLP Classifier (AUC = {roc_auc:.4f})",
        color="#e74c3c",
    )
...
def plot_precision_recall_curve(y_true, y_pred_proba, save_path=None):
    ...
    plt.plot(
        ...
        precision,
        linewidth=2,
        label=f"MLP Classifier (AP = {avg_precision:.4f})",
        color="#2ecc71",
    )
...
def plot_threshold_analysis(y_true, y_pred_proba, save_path=None):
    ...
    plt.title(
        "Model Performance Across Different Thresholds", fontsize=14, fontweight="bold"
    )
    plt.legend()
    plt.grid(True, alpha=0.3)
    plt.tight_layout()
...
def plot_calibration_curve(y_true, y_pred_proba, n_bins=10, save_path=None):
    ...
    plt.plot(
        [0, 1], [0, 1], "k--", linewidth=1, label="Perfectly Calibrated", alpha=0.5
    )
...
def calculate_business_metrics(
    y_true, y_pred, y_pred_proba, churn_cost=500, retention_cost=50, success_rate=0.3
):
    """
    ...
    total_cost_with_model = (
        retention_campaign_cost + unavoidable_churn + unsuccessful_retention
    )

    # Savings
    cost_savings = baseline_cost - total_cost_with_model
    roi = (
        (cost_savings / retention_campaign_cost) * 100
...
def plot_business_impact(business_metrics, save_path=None):
    ...
    costs = [
        business_metrics["baseline_cost"],
        business_metrics["total_cost_with_model"],
    ]
    colors = ["#e74c3c", "#27ae60"]

    axes[0].bar(
        categories, costs, color=colors, alpha=0.7, edgecolor="black", linewidth=2
    )
    ...
    axes[1].text(0.5, 0.7, "Cost Savings", ha="center", fontsize=16, fontweight="bold")
    axes[1].text(
        0.5,
        0.5,
        f"${savings:,.0f}",
    ...
    axes[1].text(
        0.5, 0.3, f"ROI: {roi:.1f}%", ha="center", fontsize=18, color="#2ecc71"
    )
    axes[1].text(
        0.5,
        0.15,
        f'Customers Saved: {business_metrics["customers_saved"]:.0f}',
        ha="center",
        fontsize=12,
    )
    axes[1].set_xlim(0, 1)
...
def plot_error_analysis(y_true, y_pred, y_pred_proba, save_path=None):
    ...
    axes[0].hist(
        y_pred_proba[errors], bins=20, alpha=0.7, color="#e74c3c", edgecolor="black"
    )
    axes[0].axvline(
    ...
    axes[0].set_title(
        "Error Distribution by Confidence", fontsize=14, fontweight="bold"
    )
    axes[0].legend()
    axes[0].grid(axis="y", alpha=0.3)
    ...
    print(
        f"\nFalse Positive Avg Confidence: {fp_proba.mean():.4f}"
        if len(fp_proba) > 0
        else ""
    )
    print(
        f"False Negative Avg Confidence: {(1-fn_proba).mean():.4f}"
        if len(fn_proba) > 0
        else ""
    )
    print("=" * 60)
...
def generate_comprehensive_report(
    metrics, advanced_metrics, business_metrics, y_true, y_pred, save_path=None
):
    """
    ...
    print(
        f"Cost with Model:               ${business_metrics['total_cost_with_model']:,.2f}"
    )
...
def main():
    ...
    model = MLPClassifier(
        input_dim=input_dim, hidden_dims=[128, 64, 32], dropout_rate=0.3
    )
    ...
    plot_precision_recall_curve(
        y_test_np, y_pred_proba, save_path="precision_recall_curve.png"
    )
    ...
    plot_prediction_distribution(
        y_test_np, y_pred_proba, save_path="prediction_distribution.png"
    )
    ...
    generate_comprehensive_report(
        ...
        y_test_np,
        y_pred,
        save_path="comprehensive_evaluation_report.txt",
    )
