...
class ChurnPredictionPipeline:
    ...
    def step_1_load_data(self):
        ...
        joblib.dump(
            preprocessor.label_encoders, f"{self.output_dir}/models/label_encoders.pkl"
        )
    ...
    def step_2_feature_engineering(self):
        ...
        if use_smote or use_feature_selection:
            ...
            engineer.plot_feature_importance(
                top_n=10, save_path=f"{self.output_dir}/plots/feature_importance.png"
            )
    ...
    def step_3_train_model(self):
        ...
        print("✓ Artifacts synced to artifacts/ for serving")
    ...
    def step_4_evaluate_model(self):
        ...
        plot_confusion_matrix(
            y_test_np, y_pred, save_path=f"{plot_dir}/confusion_matrix.png"
        )
        plot_roc_curve(y_test_np, y_pred_proba, save_path=f"{plot_dir}/roc_curve.png")
        plot_precision_recall_curve(
            y_test_np, y_pred_proba, save_path=f"{plot_dir}/precision_recall_curve.png"
        )
        plot_threshold_analysis(
            y_test_np, y_pred_proba, save_path=f"{plot_dir}/threshold_analysis.png"
        )
        plot_calibration_curve(
            y_test_np, y_pred_proba, save_path=f"{plot_dir}/calibration_curve.png"
        )
        plot_prediction_distribution(
            y_test_np, y_pred_proba, save_path=f"{plot_dir}/prediction_distribution.png"
        )
        plot_error_analysis(
            y_test_np, y_pred, y_pred_proba, save_path=f"{plot_dir}/error_analysis.png"
        )
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Run end-to-end churn prediction pipeline"
    )
    ...
    parser.add_argument(
        "--no-smote", action="store_true", help="Disable SMOTE balancing"
    )
    parser.add_argument(
    ...
    config_path = (
        f"configs/pipeline_config_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
    )
