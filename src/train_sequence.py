...
class SequenceMTLTrainer:
    ...
    def train(
        ...
        ctx = (
            mlflow.start_run(run_name="Sequence_MTL_Churn_CLV_Training", nested=True)
            if self.use_mlflow
        ...
        with ctx:
                ...
                mlflow.log_params(
                    {
                        ...
                        "use_lstm": hasattr(self.model.backbone, "lstm")
                        or isinstance(self.model.backbone, torch.nn.LSTM),
                    }
                )
            ...
            for epoch in range(epochs):
                ...
                # Validate
                val_total_loss, val_churn_loss, val_clv_loss, val_acc, val_clv_mse = (
            ...
            if self.use_mlflow:
                mlflow.log_metrics(
                    {
                        ...
                        "final_val_accuracy": self.val_accuracies[
                            np.argmin(self.val_losses)
                        ],
                        "final_val_clv_mse": self.val_clv_mse[
                            np.argmin(self.val_losses)
                        ],
                ...
                try:
                    mlflow.pytorch.log_model(
                        self.model, "sequence_mtl_churn_clv_classifier"
                    )
...
def main():
    ...
    print("=" * 80)
    trainer = SequenceMTLTrainer(
        model, device, CHURN_WEIGHT, CLV_WEIGHT, use_mlflow=not args.no_mlflow
    )
...
def plot_sequence_training_curves(history, model_type="lstm"):
    ...
    # Churn loss
    axes[0, 1].plot(
        history["train_churn_losses"], label="Train Churn Loss", linewidth=2
    )
    ...
    plt.tight_layout()
    plt.savefig(
        f"sequence_mtl_{model_type}_training_curves.png", dpi=300, bbox_inches="tight"
    )
    print(
        f"Sequence MTL training curves saved to sequence_mtl_{model_type}_training_curves.png"
    )
    plt.close()
