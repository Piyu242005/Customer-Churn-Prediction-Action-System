...
class ModelComparator:
    ...
    def plot_model_comparison(
        self, metrics=["accuracy", "precision", "recall", "f1_score"], save_path=None
    ):
        """
        ...
        for i, metric in enumerate(metrics[:4]):
            if metric in results_df.columns:
                ...
                colors = [
                    "#e74c3c" if "MLP" in idx else "#3498db" for idx in plot_data.index
                ]
    ...
    def generate_comparison_report(self, save_path=None):
        ...
        print("-" * 60)
        print(
            f"{'Rank':<6} {'Model':<25} {'Accuracy':<10} {'F1-Score':<10} {'ROC-AUC':<10}"
        )
        print("-" * 60)
        ...
        for i, (model_name, row) in enumerate(results_df.iterrows(), 1):
            ...
            print(
                f"{i:<6} {model_name:<25} {row['accuracy']:.4f}     "
    ...
    def create_ensemble(self, top_n=3):
        ...
        # Create ensemble
        estimators = [
            (name, self.models[name]) for name in top_models if name in self.models
        ]
...
def run_comprehensive_comparison(
    ...
    comparator.plot_metric_heatmap(save_path=f"{save_dir}metric_heatmap.png")
    ...
    comparator.plot_roc_comparison(
        models_dict=comparator.models, save_path=f"{save_dir}roc_comparison.png"
    )

    # Generate report
    comparator.generate_comparison_report(save_path=f"{save_dir}comparison_report.txt")
...
def main():
    ...
    comparator, results_df = run_comprehensive_comparison(
        X_train, X_test, y_train, y_test, mlp_model=model
    )
