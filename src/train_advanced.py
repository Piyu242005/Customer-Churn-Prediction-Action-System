...
class AdvancedTrainingPipeline:
    ...
    def get_default_config(self, model_type):
        ...
        if model_type == "mlp":
            base_config.update({"hidden_dims": [128, 64, 32], "dropout_rate": 0.3})
        elif model_type == "mtl":
            base_config.update(
                {
    ...
    def create_model(self, input_dim):
        ...
        if self.model_type == "mlp":
            return create_model(
                input_dim,
                self.config.get("hidden_dims", [128, 64, 32]),
                self.config.get("dropout_rate", 0.3),
            )

        elif self.model_type == "mtl":
            return create_mtl_model(
                input_dim,
                self.config.get("hidden_dims", [128, 64, 32]),
                self.config.get("dropout_rate", 0.3),
            )
    ...
    def train_model(self):
        ...
        print(
            f"Creating {self.model_type.upper()} model with input dimension: {input_dim}"
        )
    ...
    def compare_models(self, model_types=["mlp", "mtl", "sequence_lstm"]):
        ...
        for model_type in model_types:
            ...
            try:
                ...
                results[model_type] = {
                    ...
                    "final_val_accuracy": history["val_accuracies"][
                        np.argmin(history["val_losses"])
                    ],
                }

                if "clv_mse" in history:
                    results[model_type]["final_val_clv_mse"] = history["val_clv_mse"][
                        np.argmin(history["val_losses"])
                    ]
        ...
        for model_type, result in results.items():
            ...
            if "final_val_clv_mse" in result:
                comparison_results[model_type]["final_val_clv_mse"] = result[
                    "final_val_clv_mse"
                ]
...
def parse_arguments():
    ...
    parser = argparse.ArgumentParser(
        description="Advanced Churn Prediction Training Pipeline"
    )
...
def main():
    ...
    if args.model_type == "compare" or args.compare_models:
        ...
        print(
            f"Final Validation Accuracy: {history['val_accuracies'][np.argmin(history['val_losses'])]:.4f}"
        )
        if "clv_mse" in history:
            print(
                f"Final Validation CLV MSE: {history['val_clv_mse'][np.argmin(history['val_losses'])]:.4f}"
            )
