...
class SequenceDataPreprocessor:
    ...
    def load_data(self):
        ...
        if self.data_path.endswith(".db") or self.data_path.startswith("sqlite:///"):
            ...
            db_url = (
                self.data_path
                if self.data_path.startswith("sqlite:///")
                else f"sqlite:///{self.data_path}"
            )
    ...
    def engineer_features(self):
        ...
        self.df["days_since_prev_order"] = (
            self.df.groupby("Customer_ID")["Order_Date"].diff().dt.days
        )
        ...
        self.df["cumulative_revenue"] = self.df.groupby("Customer_ID")[
            "Revenue"
        ].cumsum()
        self.df["cumulative_profit"] = self.df.groupby("Customer_ID")["Profit"].cumsum()

        # Create rolling averages
        self.df["rolling_avg_revenue_3"] = (
            self.df.groupby("Customer_ID")["Revenue"]
            .rolling(3, min_periods=1)
            .mean()
            .reset_index(0, drop=True)
        )
        self.df["rolling_avg_profit_3"] = (
            self.df.groupby("Customer_ID")["Profit"]
            .rolling(3, min_periods=1)
            .mean()
            .reset_index(0, drop=True)
        )
        ...
        self.df["revenue_change"] = (
            self.df.groupby("Customer_ID")["Revenue"].diff().fillna(0)
        )
    ...
    def create_sequences(self):
        ...
        sequence_features = [
            ...
            "cumulative_revenue",
            "cumulative_profit",
            "rolling_avg_revenue_3",
            "rolling_avg_profit_3",
            "revenue_change",
            "Region_encoded",
        ...
        for customer_id, customer_data in self.df.groupby("Customer_ID"):
            ...
            for i in range(len(feature_matrix) - self.seq_length):
                ...
                if not is_last_transaction:
                    ...
                    current_date = customer_data.iloc[i + self.seq_length - 1][
                        "Order_Date"
                    ]
                ...
                clv_target = customer_data.iloc[i + self.seq_length][
                    "cumulative_profit"
                ]
                ...
                targets.append(
                    {
                        ...
                        "clv": clv_target,
                        "next_revenue": (
                            target_features[3] if len(target_features) > 3 else 0
                        ),
                        "next_profit": (
                            target_features[4] if len(target_features) > 4 else 0
                        ),
                    }
                )
                customer_ids.append(customer_id)
                sequence_dates.append(
                    customer_data.iloc[i + self.seq_length - 1]["Order_Date"]
                )

        print(
            f"Created {len(sequences)} sequences from {len(set(customer_ids))} customers"
        )
    ...
    def engineer_churn_label_from_sequences(self, targets):
        ...
        clv_values = (clv_values - clv_values.min()) / (
            clv_values.max() - clv_values.min()
        )  # Normalize
    ...
    def split_and_scale(
        self, sequences, churn_labels, clv_values, test_size=0.2, random_state=42
    ):
        """
...
def load_and_preprocess_sequence_data(
    data_path, seq_length=10, min_transactions=5, test_size=0.2, random_state=42
):
    """
...
def analyze_sequence_data(sequences, targets, customer_ids):
    ...
    print(
        f"Average sequences per customer: {len(sequences) / len(set(customer_ids)):.2f}"
    )
...
if __name__ == "__main__":
    ...
    try:
        (
            ...
            sequence_dates,
            targets,
        ) = load_and_preprocess_sequence_data(
