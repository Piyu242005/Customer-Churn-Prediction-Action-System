...
class ChurnEnsemble:
    ...
    def add_model(
        self, model: nn.Module, weight: float = 1.0, metadata: Optional[Dict] = None
    ):
    ...
    def predict_proba(
        self, x: torch.Tensor, x_seq: Optional[torch.Tensor] = None
    ) -> torch.Tensor:
        """
...
def create_default_ensemble(
    artifacts_dir: str = "artifacts", device: str = "cpu"
) -> ChurnEnsemble:
